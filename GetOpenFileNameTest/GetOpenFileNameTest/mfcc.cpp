...
static void PreEmphasis(LONG * pRawVoice, int iSampleNum, double * pDiffVoice); // 预增强
...
*******************************************************************************/
static void PreEmphasis(LONG * pRawVoice, int iSampleNum, double * pDiffVoice)
{ 
...
static double * JudgeTrainFrame2(double * pDiffVoice, int iSampleNum, int train_frame_num)
{
	...
	//int iFrameNum = iSampleNum >> 8;   // 帧总数=2*(样本数/256)-1
	...
	{
		if (iFrame + offsetIndex > iFrameNum)
		{
			free(pTranDiff);
			pTranDiff = NULL;
			pSave = NULL;
		...
		{
			//if (iFrameNum - iFrame < train_frame_num - iNum || (iFrame >= (train_frame_num * 3 / 4) && iNum <= train_frame_num * 2 / 3))
...
*******************************************************************************/
BOOL voiceToMFCC(LONG * pRawVoice, int iSampleNum, double ** MFCC, int train_frame_num)
{
